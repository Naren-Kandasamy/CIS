...
async def _resolve_crime_sub_head(crime_type: str) -> str:
    """Resolve a free-text crime type to its CSH id, if not already one."""
    if crime_type.startswith("CSH"):
        return crime_type
    try:
...
async def get_trust_weight(edge_type: str, crime_type: Optional[str]) -> float:
    if crime_type:
        resolved_crime_type = await _resolve_crime_sub_head(crime_type)
        narrow_key = _narrow_scope_key(edge_type, resolved_crime_type)
...
async def record_feedback_event(event: CorrectionEvent):
    ...
    # the existing lock registry (shared/catalyst_client.py).
    ...
    if event.crime_type:
        resolved_crime_type = await _resolve_crime_sub_head(event.crime_type)
        narrow_key = _narrow_scope_key(event.edge_type, resolved_crime_type)
...
async def _apply_session_penalty(session_id: str, edge_id: str):
    ...
    async with get_session_lock(session_id):
        ...
        penalized_ids = set(json.loads(existing["value"])) if existing and "value" in existing else set()
