...
async def build_dag(intent_object: dict) -> list:
    ...
    # This used to splice json.dumps(intent_object) straight into the prompt
    # with no structural signal marking it as data rather than instructions.
    # Same delimiter pattern as shared/ner_prompt.py: a random per-request
    # marker pair the query text can't forge a matching close-marker for.
