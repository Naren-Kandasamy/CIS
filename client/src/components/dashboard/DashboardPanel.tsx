...
export default function DashboardPanel({ visualization, evidence, authToken }: DashboardPanelProps) {
  const [activeTab, setActiveTab] = useState<'analytics' | 'graph' | 'hypothesis' | 'alerts'>('analytics');
  ...
  return (
    <div className="overflow-y-auto h-full w-full flex flex-col gap-6 animate-fade-in" style={{ padding: '40px', color: 'var(--text-primary)' }}>
      ...
      {/* Tab Navigation */}
      <div style={{ display: 'flex', gap: '4px', borderBottom: '2px solid var(--paper-line)', paddingBottom: '0px' }}>
        {(['analytics', 'graph', 'hypothesis', 'alerts'] as const).map(tab => (
          <button
            key={tab}
            type="button"
            ...
            style={{
              ...
              border: activeTab === tab ? '1px solid var(--paper-line)' : '1px solid transparent',
              borderBottom: activeTab === tab ? '2px solid var(--bg-primary)' : 'none',
          ...
          >
            {tab === 'alerts' ? '⚑ Alerts' : tab.charAt(0).toUpperCase() + tab.slice(1)}
          </button>
        ))}
      </div>
        ...
        <div className="dossier-panel dossier-paperclip" style={{ padding: '28px' }}>
          ...
          <div className="graph-split-container">
            <div className={`graph-split-graph ${graphEntity ? 'graph-split-graph--narrow' : ''}`}>
              <NetworkGraph elements={visualization?.cytoscape?.elements} onNodeClick={openGraphPanel} evidence={evidence} />
            </div>
          ...
          <div className="dossier-panel dossier-paperclip" style={{ padding: '28px', marginTop: '20px' }}>
      ...
      {/* Alerts Tab */}
      {activeTab === 'alerts' && (
        <div className="dossier-panel" style={{ padding: '28px', minHeight: '400px' }}>
          <ReviewQueuePanel
            authToken={authToken ?? null}
            onOpenFir={(firId) => {
              // Navigate to graph tab and optionally filter — placeholder hook
              setActiveTab('graph');
            }}
          />
        </div>
