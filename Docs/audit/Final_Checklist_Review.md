...
- [x] **CDR & Financial Trail Integration**: Build the pluggable data source provider pattern, `CALLED`/`TRANSFERRED` edges, and inject synthetic layering/burner patterns. *(Consult: `Docs/PS1_Extended_Investigative_Capabilities.md` - Section 2)*
- [x] **Proactive Cold-Case Matching**: Extend `SHARED_MO` ingestion to flag open/cold cases with high similarity to new FIRs via the review queue. *(Consult: `Docs/PS1_Extended_Investigative_Capabilities.md` - Section 3)*
- [x] **Confidence Engine "Contradicted" State**: Log HIGH/MEDIUM confidence claims and flag them as contradicted if new evidence/exclusion records surface later. *(Consult: `Docs/PS1_Extended_Investigative_Capabilities.md` - Section 4)*
- [x] **Vehicle/ANPR Cross-Referencing**: Ingest synthetic ANPR plate reads and cross-check against a wanted-vehicle registry. *(Consult: `Docs/PS1_Extended_Investigative_Capabilities.md` - Section 5)*
- [x] **Hypothesis Workspace**: Allow officers to log investigative theories (`HypothesisRecord`) and run deterministic checks for new supporting/contradicting evidence. *(Consult: `Docs/PS1_Extended_Investigative_Capabilities.md` - Section 7)*
...
## Phase 6: Refinement Phase Execution Roadmap (All Open Items)
*All remaining open items have been reorganized here in order of execution. We start with simple debugging and UI cleanup, then move to heavy architectural integrations.*

### Step 1: Debug Audit & Cleanup
- [ ] **Voice Module Debug & Audit**: Ensure the hotfixed voice module (`WavRecorder`) is in proper working condition without browser errors.
- [ ] **UI Refinements (Dashboard)**: Debug and fix UI issues, specifically improving the dashboard's functionality and responsiveness.
...
- [ ] **Remove CIS Console Sandbox**: Remove the 'CIS console' view from `client/src/App.tsx` and delete the deprecated `CISDashboard` component as it is no longer needed.
- [ ] **Email Support**: Contact `support@zohocatalyst.com` about exact Qwen/Zia LLM-specific rate limits to understand thresholds.

### Step 2: Voice & Language Layer v2
...
### Step 3: Department Hierarchy & RBAC
- [ ] **RBAC / Jurisdiction Scoping**: Add an `OfficerProfile` and default queries to home-district scope unless overridden or rank is elevated. *(Consult: `Docs/PS1_Extended_Investigative_Capabilities.md` & `Docs/PS1_RBAC_Case_Access_v1.md`)*
...
### Step 4: Multi-Case / Thread Management
- [ ] **Multi-Case / Thread Management**: Implement UI and backend support for handling multiple investigations/cases simultaneously rather than a single chat thread. (Requires implementing the Case Sidebar and Case Board).

### Step 5: Pre-Demo & Final Evaluation
- [ ] **Confidence Calibration**: Measure the Confidence Engine against the blind labels to ensure the `HIGH` confidence tier is >= 85% accurate.
