...
- [ ] **Open Question: Distributed Ledger vs. Centralized Hash Chain** — Evaluate if a lightweight, asynchronously-validated distributed blockchain (e.g., one node per district/station) or WORM-storage backup should replace/augment the centralized Catalyst hash chain to protect against DBA-level deletion. See Architecture document for options (Centralized, Distributed, Hybrid).
