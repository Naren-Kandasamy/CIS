...
```
CATALYST_API_TOKEN=
CATALYST_ORG_ID=                # NEW -- e.g. 60075634347, used for Zia header auth (Sec 5)
...
# --- unchanged from v8: llm_complete, vlm_extract, kb_upload, kb_search,
#     ztsql_query, ztsql_execute all stay exactly as they were ---
...
async def tag_and_normalize_language(fir: FIRSchema) -> FIRSchema:
    """Runs once per FIR, before the three-destination write, so all three
    destinations receive already-normalized data. The vast majority of FIRs
    should be fully tagged by the time they leave this function -- the
    Sec 11 conditional edge exists only for records that bypass this path."""
...
async def ingest_one(fir: FIRSchema):
    ...
    await asyncio.gather(
        upload_fir_to_kb(fir),        # now gets canonical/translated text for indexing
        write_fir_to_memgraph(fir),   # unchanged structural data
        write_fir_to_ztsql(fir)       # stores both original and canonical text
    )
```

*(Derived edge computation, edge pruning, KB document format, activityScore methodology, bias audit, and ingestion run order are all unchanged from v8 — not touched by any addendum.)*
