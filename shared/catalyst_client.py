...
async def _nosql_request(method: str, path: str, json_body) -> dict:
    ...
    last_error = None
    max_attempts = 6
    # Transient upstream statuses worth retrying. 401 is handled separately
    # below (bust the token cache, retry) rather than listed here.
    RETRYABLE_STATUS = {429, 500, 502, 503, 504}
    for attempt in range(max_attempts):
        # Re-fetch headers each attempt so a token refreshed by another
        # coroutine (or by the 401 handler below) is picked up on retry.
        headers = await _nosql_headers()
        try:
            async with httpx.AsyncClient() as client:
                r = await client.request(method, url, headers=headers, json=json_body, timeout=15.0)
                if r.status_code == 401:
                    # Cached token is stale/revoked -- invalidate and retry so
                    # the next _nosql_headers() call forces a refresh.
